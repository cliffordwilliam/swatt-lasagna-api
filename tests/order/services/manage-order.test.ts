...
describe("MANAGE_ORDER", () => {
  ...
  describe("getById", () => {
    it("returns normalized order payload", async () => {
      ...
      expect(result.orderItems).toEqual([{ itemId: 7, quantity: 3 }]);
    });
  });
  ...
  describe("create", () => {
    ...
    it("skips flush when flush flag is false", async () => {
      ...
      expect(result.orderItems).toEqual([{ itemId: 1, quantity: 1 }]);
    });
  });
  ...
  describe("update", () => {
    it("recomputes totals, replaces items, upserts people, and normalizes response", async () => {
      ...
      expect(result.orderItems).toEqual([{ itemId: 9, quantity: 4 }]);
    ...
    it("leaves items and people untouched when updates omit them and skips flush when false", async () => {
      ...
      const existingOrder = {
        orderId,
        po: "PO-300",
        orderItems: new Set([{ item: { itemId: 5 }, quantity: 2 }]) as any,
        totalPurchase: 50,
        shippingCost: 10,
        buyer: { personId: 5 },
      ...
      expect(result.orderItems).toEqual([{ itemId: 5, quantity: 2 }]);
    });
  });
